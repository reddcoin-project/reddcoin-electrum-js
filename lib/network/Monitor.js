...
var Monitor = stampit().enclose(function () {
    ...
    this.broadcastTransaction = function(signedTransaction, accountIndex, highfees=false) {
        
        if (highfees){
            console.log("%cbroadcastTransaction: highfees" + highfees, "color:blue;");
            enqueueRequest(that.types.broadcast, [signedTransaction, highfees], accountIndex);    
        } else {
